...
<style>
  .ing-head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px dashed var(--line);padding-bottom:12px;margin-bottom:16px}
  .ing-head h3{margin:0;border:none;padding:0}
  .portion-stepper{display:flex;align-items:center;gap:10px;font-family:var(--font-mono);font-size:.82rem;border:2px solid var(--ink);border-radius:999px;padding:4px 6px}
  .portion-stepper button{width:26px;height:26px;border-radius:50%;border:1px solid var(--ink);background:none;font-size:1rem;cursor:pointer;line-height:1}
  .portion-stepper button:hover{background:var(--ink);color:var(--paper)}
  ...
  .ing-nom{font-size:.85rem;margin-bottom:2px}
  .ing-qte{font-family:var(--font-mono);font-weight:700;font-size:.9rem}
  .ing-qte .qte-unite{font-weight:400;text-transform:lowercase}
...
<section class="recipe-hero">
  <div class="wrap" style="display:block">
    <span class="tag" style="color:var(--tomato);font-family:var(--font-mono);text-transform:uppercase;font-size:.78rem;letter-spacing:.08em">
    ...
    <?php if (!empty($recette['image_url'])): ?>
      <div class="thumb-big" style="width:100%;margin-top:16px">
        <img src="uploads/<?= htmlspecialchars($recette['image_url']) ?>" alt="<?= htmlspecialchars($recette['titre']) ?>" loading="eager" fetchpriority="high" decoding="async">
      </div>
    <?php else: ?>
      <div class="thumb-big" style="width:100%;margin-top:16px">🍽</div>
    <?php endif; ?>

    <p class="lede" style="max-width:48ch;color:#3a3a35;margin-top:24px"><?= htmlspecialchars($recette['intro']) ?></p>
          ...
          <div class="ing-card">
            ...
            <div class="ing-qte">
              <?php if ($qte_nombre !== '' && is_numeric($qte_nombre)): ?>
                <span class="qte-nombre" data-base="<?= htmlspecialchars($qte_nombre) ?>"><?= htmlspecialchars($qte_nombre) ?></span><span class="qte-unite"> <?= htmlspecialchars($qte_unite) ?></span>
              <?php else: ?>
