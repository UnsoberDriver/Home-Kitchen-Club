...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $temps_prepa = trim($_POST['temps_preparation'] ?? '');
    $temps_cuisson = trim($_POST['temps_cuisson'] ?? '');
    ...
    if ($categorie_id <= 0) $erreurs[] = "Merci de choisir une catégorie.";
    if (!in_array($diff, ['Facile', 'Moyen', 'Difficile'], true)) $erreurs[] = "Difficulté invalide.";
    ...
    if (!$erreurs) {
        ...
        try {
            $stmt = $pdo->prepare("
                UPDATE recettes
                SET titre = ?, categorie_id = ?, intro = ?, temps_total = ?, temps_preparation = ?, temps_cuisson = ?, portions = ?, difficulte = ?, nouveau = ?, publie = ?, image_url = ?
                WHERE id = ?
            ");
            $stmt->execute([$titre, $categorie_id, $intro, $temps, $temps_prepa, $temps_cuisson, $portions, $diff, $nouveau, $publie, $image_nom, $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $erreurs) {
    ...
    $recette['temps_preparation'] = $_POST['temps_preparation'] ?? ($recette['temps_preparation'] ?? '');
    $recette['temps_cuisson'] = $_POST['temps_cuisson'] ?? ($recette['temps_cuisson'] ?? '');
...
<style>
  ...
  .edit-field{
    ...
    border-radius:4px;font-family:inherit;color:inherit;width:100%;
  }
  .edit-field:hover{background:rgba(30,42,31,.05)}
  .edit-field:focus{outline:none;background:#fff;box-shadow:0 0 0 2px var(--tomato)}
  select.edit-field{
    ...
    background-repeat:no-repeat;background-position:right 4px center;background-size:16px;padding-right:24px;
  }
  textarea.edit-field{resize:vertical;line-height:inherit}
  .edit-title{font-family:var(--font-display);font-size:2.6rem;letter-spacing:-0.01em;margin:10px 0 14px}
  .edit-cat{font-family:var(--font-mono);text-transform:uppercase;font-size:.78rem;letter-spacing:.08em;color:var(--tomato);font-weight:600;border:none;background:transparent}
  .edit-lede{font-size:1.08rem;max-width:48ch;color:#3a3a35}
  .meta-input{font-family:var(--font-display);font-size:1.3rem;width:100%}
  .ing-row{display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px dotted var(--line);align-items:center}
  .ing-row input{font-size:.92rem}
  .ing-row input.qty{max-width:110px;text-align:right;font-family:var(--font-mono);color:var(--sage)}
  .step-block{position:relative;padding:0 0 28px 52px;margin-bottom:6px}
  .step-title-input{font-family:var(--font-display);font-size:1.4rem;margin-bottom:6px}
  .step-text-input{color:#4c4c46;min-height:70px}
  .step-num{position:absolute;left:0;top:0;width:36px;height:36px;border-radius:50%;border:2px solid var(--ink);display:flex;align-items:center;justify-content:center;font-family:var(--font-mono);font-weight:700;background:var(--paper)}
  .remove-row{background:none;border:1px solid var(--tomato);color:var(--tomato);padding:4px 10px;border-radius:999px;font-family:var(--font-mono);font-size:.7rem;cursor:pointer;white-space:nowrap}
  .remove-row:hover{background:var(--tomato);color:#fff}
  .add-mini{background:none;border:1px solid var(--ink);border-radius:999px;padding:8px 16px;font-family:var(--font-mono);font-size:.75rem;cursor:pointer;margin-top:10px}
  .add-mini:hover{background:var(--ink);color:var(--paper)}
  .admin-toolbar{background:var(--ink);color:var(--paper);border-bottom:2px solid var(--ink)}
  .admin-toolbar .wrap{display:flex;justify-content:space-between;align-items:center;padding:12px 28px;font-family:var(--font-mono);font-size:.82rem}
  .admin-toolbar a{border:1px solid var(--paper);padding:6px 14px;border-radius:999px}
  .admin-toolbar a:hover{background:var(--paper);color:var(--ink)}
  .checks-inline{display:flex;gap:20px;align-items:center}
  .checks-inline label{display:flex;align-items:center;gap:6px}
  .save-bar{position:sticky;bottom:0;background:var(--paper);border-top:2px solid var(--ink);padding:16px 0;margin-top:20px}
  .save-bar .wrap{display:flex;justify-content:space-between;align-items:center}
  .btn-save{background:var(--ink);color:var(--paper);padding:13px 28px;border-radius:999px;font-weight:600;border:2px solid var(--ink);cursor:pointer}
  .btn-save:hover{background:var(--tomato);border-color:var(--tomato)}
  .thumb-upload{
    cursor:pointer;flex-direction:column;gap:8px;color:rgba(30,42,31,.55);
    transition:background .15s;
  }
  .thumb-upload:hover{background:rgba(30,42,31,.08)}
  .thumb-upload img{width:100%;height:100%;object-fit:cover}
  .erreurs{background:#f8d7da;color:#8a1c25;padding:14px 18px;border-radius:4px;margin:20px 0}
...
<header class="site-header">
  <div class="wrap">
    <a href="dashboard" class="logo"><span class="dot"></span> Home Kitchen Club</a>
    <div style="display:flex;gap:10px;margin-left:auto">
      <a href="logout" class="nav-cta">Déconnexion</a>
    </div>
  </div>
</header>
...
<section class="recipe-hero">
  <div class="wrap" style="display:block">
    ...
    <input type="text" name="titre" class="edit-field edit-title" value="<?= htmlspecialchars($recette['titre']) ?>" placeholder="Titre de la recette">
    ...
    <label for="image-input" class="thumb-big thumb-upload" id="thumb-upload" style="width:100%;margin-top:16px">
      <span id="thumb-placeholder" style="<?= !empty($recette['image_url']) ? 'display:none' : '' ?>">
        ...
        <span style="font-family:var(--font-mono);font-size:.75rem;text-transform:uppercase;letter-spacing:.06em">Ajouter une image</span>
      </span>
      <img id="thumb-preview" src="<?= !empty($recette['image_url']) ? 'uploads/' . htmlspecialchars($recette['image_url']) : '' ?>" alt="" style="<?= !empty($recette['image_url']) ? 'display:block;object-fit:contain' : 'display:none' ?>">
    </label>
    ...
    <textarea name="intro" class="edit-field edit-lede" rows="2" placeholder="Introduction / description" style="margin-top:24px"><?= htmlspecialchars($recette['intro']) ?></textarea>

    <div class="recipe-meta">
      <div>
        <input type="text" name="temps_cuisson" class="edit-field meta-input" value="<?= htmlspecialchars($recette['temps_cuisson'] ?? '') ?>" placeholder="ex : 25 min">
        Temps de cuisson
      </div>
      <div>
        <input type="text" name="temps_preparation" class="edit-field meta-input" value="<?= htmlspecialchars($recette['temps_preparation'] ?? '') ?>" placeholder="ex : 20 min">
        Temps de préparation
      </div>
      <div>
        <input type="text" name="temps_total" class="edit-field meta-input" value="<?= htmlspecialchars($recette['temps_total']) ?>" placeholder="ex : 45 min">
        Temps total
      </div>
      <div>
        <input type="text" name="portions" class="edit-field meta-input" value="<?= htmlspecialchars($recette['portions']) ?>" placeholder="ex : 4">
        Portions
      </div>
      <div>
    ...
    <div class="checks-inline" style="margin-top:16px;font-family:var(--font-mono);font-size:.8rem">
      <label><input type="checkbox" name="nouveau" <?= !empty($recette['nouveau']) ? 'checked' : '' ?>> Marquer « Nouveau »</label>
    </div>
  </div>
</section>
...
<div class="wrap">
  <div class="recipe-body">
    <aside class="ingredients">
      ...
      <div id="ingredients-list">
        <?php foreach ($ingredients as $ing): ?>
          <div class="ing-row">
            <input type="text" name="ing_nom[]" class="edit-field" value="<?= htmlspecialchars($ing['nom']) ?>" placeholder="Nom de l'ingrédient">
            <input type="text" name="ing_qte[]" class="edit-field qty" value="<?= htmlspecialchars($ing['quantite']) ?>" placeholder="Quantité">
    ...
    <div class="steps">
      <h3>Préparation</h3>
      <div id="etapes-list">
        <?php $n = 1; foreach ($etapes as $etape): ?>
          <div class="step-block">
            <span class="step-num"><?= $n++ ?></span>
            <input type="text" name="etape_titre[]" class="edit-field step-title-input" value="<?= htmlspecialchars($etape['titre']) ?>" placeholder="Titre de l'étape">
            <textarea name="etape_texte[]" class="edit-field step-text-input" placeholder="Détail de l'étape..."><?= htmlspecialchars($etape['texte']) ?></textarea>
