...
<style>
  ...
  select.edit-field{cursor:pointer;-webkit-appearance:none;appearance:none;background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%231E2A1F' stroke-width='2'><path d='M6 9l6 6 6-6'/></svg>");background-repeat:no-repeat;background-position:right 4px center;background-size:14px;padding-right:20px}
  .card h3.edit-title{font-family:var(--font-display);font-size:1.3rem;margin:0 0 8px;font-weight:700}
  .card input.edit-title{font-family:var(--font-display);font-size:1.3rem;margin:0 0 8px;font-weight:700}
  .card .tag select.edit-field{font-family:var(--font-mono);text-transform:uppercase;font-size:.7rem;letter-spacing:.08em;color:var(--tomato);border:none;background:transparent;margin-bottom:8px}
  .meta-row{display:flex;gap:10px;font-family:var(--font-mono);font-size:.78rem}
  .meta-row input.edit-field{max-width:90px}
  .checks-inline{display:flex;gap:14px;align-items:center;font-family:var(--font-mono);font-size:.72rem;margin-top:10px}
  .checks-inline label{display:flex;align-items:center;gap:4px}
  .card-admin-actions{
    display:flex;justify-content:space-between;align-items:center;
    padding:10px 18px;border-top:1px dashed var(--line);
  }
  form.card-form{margin:0}
  .btn-save{background:var(--ink);color:var(--paper);padding:6px 14px;border-radius:999px;font-family:var(--font-mono);font-size:.72rem;border:1px solid var(--ink);cursor:pointer}
  .btn-save:hover{background:var(--tomato);border-color:var(--tomato)}
  form.delete{display:inline;margin:0}
  .btn-del{background:none;border:1px solid var(--tomato);color:var(--tomato);padding:5px 10px;border-radius:999px;font-family:var(--font-mono);font-size:.72rem;cursor:pointer}
  .btn-del:hover{background:var(--tomato);color:#fff}
  .msg{padding:12px 16px;border-radius:4px;margin:20px 0;font-size:.9rem}
  .msg.ok{background:#dff0d8;color:#2c5c2c}
  .msg.err{background:#f8d7da;color:#8a1c25}

  .card-add{
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    min-height:280px;border:2px dashed var(--ink);background:transparent;
    color:var(--ink);text-align:center;gap:10px;transition:background .15s;
  }
  .card-add:hover{background:rgba(30,42,31,.05);transform:translateY(-6px) rotate(-0.6deg)}
  .card-add-icon{
    font-family:var(--font-display);font-size:2.4rem;
    width:56px;height:56px;border-radius:50%;border:2px solid var(--ink);
    display:flex;align-items:center;justify-content:center;
  }
  .card-add-label{font-family:var(--font-mono);font-size:.82rem;text-transform:uppercase;letter-spacing:.06em}
...
<div class="cat-strip">
  <div class="wrap">
    ...
    <?php foreach ($categories as $c): ?>
      <a href="dashboard.php?categorie=<?= $c['id'] ?>" class="<?= $categorie_active === (int) $c['id'] ? 'active' : '' ?>">
...
<section class="section" id="recettes">
  <div class="wrap">
    ...
    <div class="section-head">
      <h2><?= $categorie_active > 0 ? htmlspecialchars(array_values(array_filter($categories, fn($c) => (int) $c['id'] === $categorie_active))[0]['nom'] ?? 'Recettes') : 'Toutes les recettes' ?></h2>
      <span class="num"><?= count($recettes) ?> résultats</span>
    </div>

    <div class="grid">
      <?php foreach ($recettes as $r): ?>
        <div class="card" style="position:relative;cursor:pointer" onclick="if(!event.target.closest('input,select,button,label,a')){window.location='modifier?id=<?= $r['id'] ?>'}">
          <?php if ($r['nouveau']): ?><span class="badge">Nouveau</span><?php endif; ?>
          <div class="thumb"><?php if (!empty($r['image_url'])): ?><img src="uploads/<?= htmlspecialchars($r['image_url']) ?>" alt="" style="width:100%;height:100%;object-fit:cover"><?php else: ?>🍽<?php endif; ?></div>
          <form class="card-form" method="post">
            ...
            <div class="body">
              ...
              <input type="text" name="titre" class="edit-field edit-title" value="<?= htmlspecialchars($r['titre']) ?>">
              <div class="meta-row">
                <input type="text" name="temps_total" class="edit-field" value="<?= htmlspecialchars($r['temps_total']) ?>">
          ...
          <form id="delete-<?= $r['id'] ?>" class="delete" method="post" onsubmit="return confirm('Supprimer définitivement « <?= htmlspecialchars(addslashes($r['titre'])) ?> » ?');">
            <input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['csrf']) ?>">
