 ...
 * register_admin
 * -----------------------------------------------------------
 * Script à usage unique pour créer un compte administrateur.
 * -> N'est lié depuis AUCUNE page publique — accès uniquement en tapant
 *    l'URL directement (ex: http://localhost/admin/register_admin).
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($stmt->fetch()) {
            ...
            $succes = true;
            $message = "Compte administrateur créé avec succès. Vous pouvez maintenant vous connecter, puis supprimer ce fichier (register).";
        }
    }
}
?>
...
<head>
<meta charset="UTF-8">
<title>Créer un compte admin (usage interne) — Home Kitchen Club</title>
...
<style>
  ...
  @media (max-width:600px){
    ...
    .box h1{font-size:1.9rem;margin-bottom:6px}
